...
async function save(payload: Record<string, any>) {
  ...
  try {
    ...
    auditLog({ action: editing.value?.id ? 'update' : 'create', target_type: 'leadership', target_label: data.full_name })
    toast.success('Saved')
...
async function del(row: any) {
  ...
  try { await remove(row.id)
    auditLog({ action: 'delete', target_type: 'leadership', target_id: row.id, target_label: row.full_name })
