/*
  # Use private.is_super_admin / private.admin_can in appraisal RPCs

  1. Problem
    - Previous fix called `public.is_super_admin()` which does not exist.
    - Both helpers live in the `private` schema.

  2. Changes
    - Recreate the three appraisal RPCs to gate access via
      `private.is_super_admin()` OR `private.admin_can('performance','manage')`.
*/
...
BEGIN
  IF NOT (private.is_super_admin() OR private.admin_can('performance','manage')) THEN
