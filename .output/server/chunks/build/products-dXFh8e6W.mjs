  ...
  setup(__props) {
    ...
    const filtered = computed(() => {
      ...
      return products.value.filter(
        (p) => {
          var _a, _b, _c;
          return p.name.toLowerCase().includes(q) || ((_a = p.tagline) != null ? _a : "").toLowerCase().includes(q) || ((_b = p.description) != null ? _b : "").toLowerCase().includes(q) || ((_c = p.category) != null ? _c : "").toLowerCase().includes(q);
        }
      );
    });
    const statusBadge = {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(filtered), (p) => {
          var _a;
          _push(`<article class="card card-hover p-6 flex flex-col"><div class="flex items-start justify-between gap-3 mb-3"><span class="${ssrRenderClass(["badge capitalize", (_a = statusBadge[p.status]) != null ? _a : "badge-slate"])}">${ssrInterpolate(p.status)}</span>`);
          if (p.category) {
...
//# sourceMappingURL=products-dXFh8e6W.mjs.map
